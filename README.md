...
| **Companion LoRA** | **Off** by default. On, it adds a small LoRA (rank 2, 2 epochs, learning rate 2e-4, EMA mode, each editable) trained as a real H3 run — every MiniMax-tab default applies unseen (training structure, high-noise LR, Optimised Likeness, the training adapter, EMA, Blocks Swap and Base Precision on Auto) — with the mod riding as the reference on every step and preview, on the stills that are **not** references (they are held out), and stores it **in the same file**, every epoch checkpoint included. Measured honestly on a 29-photo set it did **not** lift likeness beyond noise in two or ten epochs, on any learning rate, EMA mode, base precision, reference count, block set or noise density we tried, and it pulls poses toward the dataset's framing. It stays as an option for bigger datasets and your own tests. |
