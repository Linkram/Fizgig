...
| **Steps** | **0** (default) writes the plain encode, what the node pack's own extractor makes. Above 0 optimises the mod's latent against the frozen H3 model on the dataset's other stills — the model stays untouched, the latent learns what H3 needs to be shown to reproduce the subject. A mod-only option: measured on a 29-photo set (ArcFace likeness vs the dataset, four seeds) portraits 66 → 70 and scene prompts 53 → 59, but combined with the companion LoRA the two adjustments stack and overshoot, so leave it at 0 when the LoRA is on. |
| **Companion LoRA** | **On** by default: rank 2, 2 epochs, learning rate 2e-4, each editable. A tiny LoRA trained *with the mod in the conditioning* on the stills that are **not** references, so it learns only what a reference can't carry — the subject away from the reference's own pose, expression and scene — and it is stored **in the same file**. Measured with the plain mod: four scene prompts the references never showed (park, office, beach, snow) 53 → 63, the best generalisation of anything here; the LoRA also pulls poses toward the dataset's, so a "looking at the camera" prompt can come back in profile. |
| **References / Mod LR / Pull / Noise window** | How many dataset stills stack into the mod (photos first; a live token count sits beside it against the node pack's 5,120 default cap), and the optimiser's dials for anyone sweeping Steps. The card also lists the standard extractor's own defaults and ranges for comparison. |

Up to 8 references come from the dataset's photos first, then clip stills (the sharpest-face frame each clip already caches). Selecting the entry switches **Training Base** to **Reference (ref2va)**: a mod rides that model's reference path at generation, so it is optimised against it, and that is the checkpoint to load in ComfyUI with it (fl2va takes references too, in practice; ref2va is the model trained for them). Previews on the Samples tab show the plain mod first, then the optimised one when Steps is above 0, then the mod with its LoRA. Output is one file, `<name>.safetensors`, in the LoRA output folder — copy it to `ComfyUI/models/refmods/`.
