...
good on both picture and sound. **All Blocks** trains **6-49** on every step type: less preservation of the model's priors, still high quality, and it can affect movement ability — worth trying when face likeness is the thing you are chasing, at the cost of a slower step since the backward covers 44 blocks instead of 30. **Off** hands the blocks
to you, for experiments. **Every preset loads Default**, and the dropdown switches to All Blocks whenever you want it. In every mode the LoRA leaves the model's text token refiner
...
- **Training mode** (default **Default**) — Default is high quality, versatile and the best at preserving the model's priors: photos and clips train the identity blocks (20-49) and voice the audio zone (34-49), and the steps are quicker too; confining clips this way trains video just as well and makes clip steps far lighter on VRAM. **All Blocks** trains 6-49 on every step type for less preservation of the model's priors at a slower step, and is what the Style preset loads. **Off** hands Blocks to Train to you, for experiments.
...
- **Blocks to Train** — hand-pick a subset of H3's 50 blocks, live when Training mode is **Off** (the other modes own the choice and grey it out with a note). The measured recipes: **`6-49`** for the whole model (All Blocks), **`20-49` for likeness** (Default), voice core `38-48`. Type ranges (`3-12, 22, 31-33`) to experiment beyond them. Blocks 0-5 are in none of them: they deform anatomy and pull the dataset's colour into every render.
