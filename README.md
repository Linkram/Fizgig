...
| **Steps** | The part no other maker has. With the H3 base loaded and frozen, the mod's latent is the only thing that learns: a few hundred steps of the ordinary training loss on your captioned stills push it toward what makes H3 reproduce the subject, held close to the plain encode and worked only at the noise levels that matter. **200** by default, measured: portraits 66 → 70 and scenes the references never showed 53 → 59, ahead on every prompt. **0** writes the plain encode, the same file the pack's extractor makes — and needs no captions: the photos alone are the references, and the text-encoder step is skipped. |
