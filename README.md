...
| **Steps** | The part no other maker has. With the H3 base loaded and frozen, the mod's latent is the only thing that learns: a few hundred steps of the ordinary training loss on your captioned stills push it toward what makes H3 reproduce the subject, held close to the plain encode and worked only at the noise levels that matter. **200** by default, measured: portraits 66 → 70 and scenes the references never showed 53 → 59, ahead on every prompt. Side by side with the plain encode of the same photos, the optimised mod also renders skin with visibly less noise and fine detail sharper: a plain encode carries the photo's grain and the VAE's own error, and H3 copies all of it; the steps keep what helps the model predict the subject and let the rest go. **0** writes the plain encode, the same file the pack's extractor makes — and needs no captions: the photos alone are the references, and the text-encoder step is skipped. |
...
Selecting the entry switches **Training Base** to **Reference (ref2va)**: a mod rides that model's reference path at generation, so it is optimised against it, and that is the checkpoint to load in ComfyUI with it (fl2va takes references too, in practice). Keep the int8 base for RefMods where it fits: NF4 costs the mod about ten points before any optimisation. Previews are off by default for this family; switched on in the Samples tab, they render as 22-frame 640×768 clips with the mod as it will be used, a silent mp4 per prompt with its middle frame as the gallery image. Output lands in the LoRA output folder as `<name>.safetensors`: copy it to `ComfyUI/models/refmods/` and load it with **Load H3 RefMods → Apply H3 RefMod**. The file is a standard RefMod; nothing about it is special to their nodes.
