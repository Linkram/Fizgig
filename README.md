...
**More Blocks** trains **6-49** on every step type. That means less preservation of the model's priors and it can affect movement ability, at a slower step since the backward covers 44 blocks instead of 30. Reach for it when face likeness is specifically what you are chasing, not as a general upgrade — the name is about coverage, not quality. **Off** hands the blocks to you, for experiments. In every mode the LoRA leaves the model's text token refiner
...
- **Training mode** (default **Default**) — Default is high quality, versatile and the best at preserving the model's priors: photos, clips and voice all train the identity blocks (20-49), and the steps are quicker too; confining clips this way trains video just as well and makes clip steps far lighter on VRAM. **More Blocks** trains 6-49 on every step type for less preservation of the model's priors at a slower step, and is what the Style preset loads. **Off** hands Blocks to Train to you, for experiments.
