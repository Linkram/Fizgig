...
Fizgig makes them with the model in the loop. Pick **MiniMax H3 RefMod** in the Base Model selector and the Training tab becomes Output plus a short card:
...
| **Steps** | The part no other maker has. With the H3 base loaded and frozen, the mod's latent is the only thing that learns: a few hundred steps of the ordinary training loss on your captioned stills push it toward what makes H3 reproduce the subject, held close to the plain encode and worked only at the noise levels that matter. **200** by default, measured: portraits 66 → 70 and scenes the references never showed 53 → 59, ahead on every prompt. **0** writes the plain encode, the same file the pack's extractor makes. |
| **Target MP** | The resolution the references are encoded at. 0.25 is the H3 default and holds up. |
