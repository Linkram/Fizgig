...
| **Companion LoRA** | **Off** by default. On, it adds a small LoRA (rank 2, 2 epochs, learning rate 2e-4, EMA mode, each editable) trained as a real H3 run — every MiniMax-tab default applies unseen (training structure, high-noise LR, Optimised Likeness, the training adapter, EMA, Blocks Swap and Base Precision on Auto) — with the mod riding as the reference on every step and preview, on **every** still (references included), and stores it **in the same file**, every epoch checkpoint included. Measured on a 29-photo set: one epoch at rank 2 or 4 carries no identity on its own (reference 0 + LoRA scores like a stranger) — it adds a few points at reference 0.8 to 1.0 and needs epochs, not rank, to stand alone. The useful operating range for the pair is reference 0.8 to 1.0 with the LoRA at 1.0; below 0.6 the reference falls away faster than a short LoRA can compensate. |
