...
Fizgig makes them with the model in the loop. Pick **MiniMax H3 RefMod** in the Base Model selector and the Training tab becomes Output plus a short card. Five presets ship. For a person: the **community recipe** (plain encode, 8 references at 1 MP — the node pack's own extractor path under its token cap, the default), **Fizgig recipe lite** (Full reference, 16 references at 0.5 MP, optimised against the model — about the library's token budget at generation) and the **Fizgig recipe** (the same at 1 MP: twice the tokens per reference, so a heavier file that is slower to make and slower to generate with, for a little more sharpness, smoother skin gradients and a small edge in likeness over lite), so you can make the same photos each way and compare. For a look rather than a person: **style, community** (the pack's concept recipe — the pooled 16×16 grid of every still, light enough to stack, carrying palette and feel rather than pictures) and **style, high fidelity** (Full reference at 1 MP, 8 stills — the pixels that carry texture, grain and brushwork, at the library's token budget). Both style presets are plain encodes, so they need no captions; Steps on a style set is untried — the optimiser rewards reproducing the stills, which on a style set means their subjects and framing too — so type a number only to experiment. The card is:
...
| **References** | How many dataset stills stack into the mod, photos first, then each clip's sharpest-face frame. **16** by default: measured on a 29-photo set (ArcFace likeness against the dataset, four seeds, trigger-free prompts), 16 references scored **70** on portraits against 60 for 8, the level two real photos of the same person score against each other. A live token count sits beside it against the node pack's 5,120 default cap. |
| **Steps** | The part no other maker has. With the H3 base loaded and frozen, the mod's latent is the only thing that learns: the ordinary training loss on your captioned stills pushes it toward what makes H3 reproduce the subject, held close to the plain encode and worked only at the noise levels that matter. Each step rides one reference beside one still, so a step is about a second on a 32 GB card and **200** steps is a few minutes. What it buys, measured on same-seed clips against the dataset (16 Sep 2026): on a shot like the photos, the plain encode and the optimised mod score level — a plain encode copies the photos, and copies them well while the shot resembles them. On a look the photos never showed (the same face under clown make-up), the optimised mod scored 28 against 21, its worst frame above the plain encode's best: it carries the person, not the pictures. In both cases the optimised mod renders skin with visibly less noise and fine detail sharper — a plain encode carries the photo's grain and the VAE's own error, and H3 copies all of it; the steps keep what helps the model predict the subject and let the rest go. **0** writes the plain encode, the same file the pack's extractor makes — and needs no captions: the photos alone are the references, and the text-encoder step is skipped. |
| **Description / Concept** | The mod's hint: a few words saying what it is ("a ginger woman with messy hair", "a 1970s film look") and what kind, in the pack's terms — identity, style, pose_motion, clothing, background, generic. Both go into the file; RefMod Studio's **+ mod hints** and the pack's loader put "concept: description" into the prompt, which the pack's own guide says a mod needs to anchor on. The style presets set style. |
| **Clips** | How a clip in the dataset enters the mod. **As their sharpest still** (default): one frame per clip, the face, the identity choice. **As motion**: every latent frame of the clip, the node pack's video reference for a dance or a camera move; tokens are per frame, so a clip costs its length. Prepare clips with Gizmo first: cut to H3's frame grid at the right size, they work best. Steps on a motion mod is untried — leave it at 0 to experiment. |
| **Target MP** | The resolution the references are encoded at. 0.25 is the H3 default and holds up; 1.0 carries four times the pixels per face. With Steps above 0 this sizes the references only: the optimiser's own stills are always cached at 0.25 MP in a second, lighter pass. |

**Training Base picks the model the mod is made for, and both H3 models are covered.** Selecting the entry switches it to **Reference (ref2va)**, the model built to take references; pick **First / Last Frame (fl2va)** and the mod is optimised against that model instead. This matters: the same 16 stills optimised for ref2va and run on the first/last-frame model scored 60 against the dataset, and optimised for fl2va and run there scored 65, with the worst frame of the matched mod above the best frame of the mismatched one. Made for the model it runs on, a mod holds the person about as well on either: lite files scored 66 on ref2va and 65 on fl2va. Load the mod in ComfyUI with the model it was made for, and make one per model if you use both. There are no previews: the Samples tab is not used for this family, and a mod is judged in RefMod Studio once it is made. Output lands in the RefMod family's Output folder as `<name>.safetensors` — the Output folder is remembered per family, so point this one at `ComfyUI/models/refmods/` and every mod is ready to load with **Load H3 RefMods → Apply H3 RefMod**. The file is a standard RefMod; nothing about it is special to their nodes.

**RefMod Studio** tests a mod before it goes anywhere near ComfyUI, on either H3 model — pick Reference (ref2va) or First / Last Frame (fl2va) at the top, the same choice you make in ComfyUI. The tab is the node pack's Load H3 RefMods, Axis, Apply and Step Curve as one screen: rows of mods with a strength and copies (pick a second mod under **vs** and the row becomes an A/B axis), retention with the pack's four presets, a scramble seed, and the frame curve and step curve drawn live as you set them. Render a still for the fast loop or a clip with sound, and the base model at the same seed sits beside it as **No mod**. **Render sweep** steps one dial through its useful values (retention, seeds, curve directions, copies) and lays the stills out as a strip. When it looks right, **ComfyUI settings** gives you the exact widget values to type into the nodes, or **Bake as new RefMod** writes a copy with the strength and frame curve folded in, which loads at 1.0 in any loader. The numbers are the pack's own maths, so what you see here is what those nodes will do.
