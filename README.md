...
Fizgig makes them, from stills, with a **MiniMax H3 RefMod** entry in the Base Model selector, and by default pairs the mod with a small **companion LoRA** trained on the rest of the dataset — the references themselves are held out of its training — stored in the same file. The Training tab becomes Output plus a short card:
...
| **References** | How many dataset stills stack into the mod, photos first (the node pack takes up to 16). A live token count sits beside it against the node pack's 5,120 default cap: about 244 tokens per reference at 0.25 MP. |
...
| **Companion LoRA** | **On** by default: rank 2, 2 epochs, learning rate 2e-4, each editable. This is a real H3 training run — every MiniMax-tab default applies unseen (training structure, high-noise LR, Optimised Likeness, the training adapter, EMA, Blocks Swap and Base Precision on Auto) — with the mod riding as the reference on every step and preview, trained on the stills that are **not** references (they are held out), so the LoRA learns only what a reference can't carry: the subject away from the reference's own pose, expression and scene. It is stored **in the same file**, every epoch checkpoint included. Measured with the plain mod and a bare rank-2 loop: four scene prompts the references never showed (park, office, beach, snow) 53 → 63, the best generalisation of anything here; the LoRA also pulls poses toward the dataset's, so a "looking at the camera" prompt can come back in profile. **Off** writes the plain mod with no training run. |

Optimising the mod's latent itself against the frozen model (a modest mod-only gain, measured 66 → 70 on portraits, 53 → 59 on scenes) stays available on the command line, `minimax_refmod.py --steps N`; combined with the LoRA the two adjustments stack and overshoot, so it is not on the card.

References come from the dataset's photos first, then clip stills (the sharpest-face frame each clip already caches). Selecting the entry switches **Training Base** to **Reference (ref2va)**: a mod rides that model's reference path at generation, so it is optimised against it, and that is the checkpoint to load in ComfyUI with it (fl2va takes references too, in practice; ref2va is the model trained for them). Previews on the Samples tab render with the mod and the LoRA exactly as they will be used, every epoch. Output is `<name>.safetensors` plus the epoch checkpoints, in the LoRA output folder, each a complete pair — copy one to `ComfyUI/models/refmods/`.
