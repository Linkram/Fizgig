...
**More Blocks** trains **6-49** on every step type. That means less preservation of the model's priors and it can affect movement ability, at a slower step since the backward covers 44 blocks instead of 30. The name is about coverage, not quality, so it is not a general upgrade — but there are two cases where it is the right pick. Reach for it when face likeness is specifically what you are chasing, and reach for it when you are **training a motion**: loosening the model's movement priors is a cost only when you want to keep them, and if movement is the thing you are teaching, those are exactly the priors you are replacing. **Off** hands the blocks to you, for experiments. In every mode the LoRA leaves the model's text token refiner
...
- **Training mode** (default **Default**) — Default is high quality, versatile and the best at preserving the model's priors: photos, clips and voice all train the identity blocks (20-49), and the steps are quicker too; confining clips this way trains video just as well and makes clip steps far lighter on VRAM. **More Blocks** trains 6-49 on every step type (the pick for motion LoRAs, and worth trying for face likeness) for less preservation of the model's priors at a slower step, and is what the Style preset loads. **Off** hands Blocks to Train to you, for experiments.
