...
**Training mode** picks the recipe, and **Default** is the one to use. It is high quality, versatile, and the best of the three at preserving what the model already knows — photos and clips train the identity blocks (**20-49**), voice the audio zone (**34-49**), and because the backward stops at the window the steps are quicker as well. Styles train both faster and better on it, which is why every preset now ships it.
...
- **Training mode** (default **Default**) — Default is high quality, versatile and the best at preserving the model's priors: photos and clips train the identity blocks (20-49) and voice the audio zone (34-49), and the steps are quicker too; confining clips this way trains video just as well and makes clip steps far lighter on VRAM. **More Blocks** trains 6-49 on every step type for less preservation of the model's priors at a slower step, and is what the Style preset loads. **Off** hands Blocks to Train to you, for experiments.
