...
good on both picture and sound. **More Blocks** trains **6-49** on every step type: less preservation of the model's priors, still high quality, and it can affect movement ability — worth trying when face likeness is the thing you are chasing, at the cost of a slower step since the backward covers 44 blocks instead of 30. **Off** hands the blocks
to you, for experiments. **Every preset loads Default**, and the dropdown switches to More Blocks whenever you want it. In every mode the LoRA leaves the model's text token refiner
