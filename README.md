...
Fizgig makes them, from stills, with a **MiniMax H3 RefMod** entry in the Base Model selector. The Training tab becomes Output plus a short card:
...
| **References** | How many dataset stills stack into the mod, photos first. **16** by default, the node pack's own maximum: measured on a 29-photo set (ArcFace likeness vs the dataset, four seeds, trigger-free prompts), 16 references scored **70** on portraits against 60 for 8 — the level two real photos of the same person score against each other. A live token count sits beside it against the node pack's 5,120 default cap: about 244 tokens per reference at 0.25 MP. |
| **Target MP** | The dataset resolution the LoRA half trains at — the one MiniMax-tab default kept in view. 0.25 is the H3 default and holds up. |
| **Companion LoRA** | **Off** by default. On, it adds a small LoRA (rank 2, 2 epochs, learning rate 2e-4, EMA mode, each editable) trained as a real H3 run — every MiniMax-tab default applies unseen (training structure, high-noise LR, Optimised Likeness, the training adapter, EMA, Blocks Swap and Base Precision on Auto) — on **every** still, as an ordinary LoRA run — no reference in the conditioning while it trains, so it learns the face itself, exactly what the MiniMax tab would train — and stores it **in the same file** as the mod, every epoch checkpoint included. Previews render the pair as the Fizgig node applies it, the mod at the reference strength you set and the LoRA at 1.0. (Training the LoRA *with* the mod in the conditioning, so it learns only what the reference doesn't carry, is kept as a command-line experiment; measured, it produced a LoRA with no identity of its own.) |

Optimising the mod's latent itself against the frozen model (a modest mod-only gain, measured 66 → 70 on portraits, 53 → 59 on scenes, with run-to-run variance about the same size) stays available on the command line, `minimax_refmod.py --steps N`. NF4 as the base costs the mod itself about ten points before any training; keep the int8 base for RefMods where it fits.

References come from the dataset's photos first, then clip stills (the sharpest-face frame each clip already caches). Selecting the entry switches **Training Base** to **Reference (ref2va)**: a mod rides that model's reference path at generation, so it is optimised against it, and that is the checkpoint to load in ComfyUI with it (fl2va takes references too, in practice; ref2va is the model trained for them). With the LoRA off the run is seconds: encode, write, done, plus previews from the Samples tab if enabled. With it on, previews render with the mod and the LoRA exactly as they will be used, every epoch, and the epoch checkpoints are pair files too. Output lands in the LoRA output folder as `<name>.safetensors` — copy it to `ComfyUI/models/refmods/`.
