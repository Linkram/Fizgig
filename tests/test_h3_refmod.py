...
ck("train_minimax: builds the mod from the caches, trains on every still by default, rides it on every step and preview, attaches it to every save",
   ...
   and _ts.count("ref_latents=([_refmod") == 2 and "refmod_train_on_refs: bool = True" in _ts and "refmod_train_with_ref: bool = False" in _ts and _ts.count("_attach_refmod(") >= 3)
