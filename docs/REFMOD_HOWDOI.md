...
- **Token cap.** Off keeps every frame, which is how the person presets are measured. Set a cap and the mod is thinned the way the pack's extractor thins its own: only when it is over the cap, frames that are near-duplicates of the last kept one go first (a held shot in a clip is mostly those), then what is left is spread evenly down to the count that fits. Worth setting for clips as motion, where two similar clips would otherwise carry the same frames twice. The log says what was dropped.
