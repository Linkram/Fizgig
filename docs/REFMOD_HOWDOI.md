...
4. **Render.** Set the prompt (press **+ mod hints** to append each mod's hint), seed, length, size, steps and Turbo, then **Render**. Tick **Numbered references** and the mods are shown to the text encoder as numbered pictures, the way the pack's H3 RefMod Text Encode node presents saved references, so the prompt can say "the woman in <Picture 1> stands on the left"; the map beside the tick says which number is which and **+ labels** drops them into the prompt. For now every mod is presented as a Picture, its first frame; the pack labels a stacked-photo mod <Video n>, so change the word when you take a prompt to ComfyUI, and audio mods are not presented. With mods appears on the left the moment it is done, No mod on the right for comparison. **Render sweep** steps one dial through its useful values and gives you a row of 22-frame clips to compare; click a chip to play it.
